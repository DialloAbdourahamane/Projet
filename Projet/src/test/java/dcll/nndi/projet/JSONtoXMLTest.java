...
public class JSONtoXMLTest {
     ...
     * fichier contenant le resultat a bien été crée et non vide
    ...
    @Test
    /**
     * un autre test sur la methode permettant de parser le fichier au format xml
     * on fait appel à la methode convertJsonToXml puis on verifie que le 
     * fichier contenant le resultat a bien été crée.
     * @throws XMLStreamException
     * @throws IOException
     */
    public void testconvertJsonToXml1() {
        File fich=null;
        try {
            testfichier.convertJsonToXml("fichier.json", "sortie.xml");
            fich = new File("sortie.xml");
            assertTrue(fich.getTotalSpace()!=0);
        } catch (Exception e) {
